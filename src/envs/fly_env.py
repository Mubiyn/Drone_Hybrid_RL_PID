...
class QuadcopterEnv(BaseRLAviary):
    ...
    def step(self, action):
        ...
        # --------------------------------------
        # PHASE 1: ABSOLUTE HOVER
        # --------------------------------------
        if self.phase == "hover":
            target = np.array([0, 0, 1.0])   # Force hover
        ...
        # Scale the action (IMPORTANT)
        #delta = delta * 0.4    # enlarge movements

        target = pos + delta
        target[2] = 1.0        # ALWAYS enforce altitude during circle
    ...
    def _computeObs(self):
        ...
        return np.concatenate([pos, vel, rpy, ang_vel, rel_wp]).astype(np.float32)
    ...
    def _computeReward(self):
        ...
        reward += 3.0 * (self.prev_dist - dist)# progress only
        ...
        reward -= 4.0 * abs(pos[2] - 1.0)# STRONG altitude hold
        # EXTRA penalty for going too low
        if pos[2] < 0.9:
            reward -= 3.0 * (1.0 - pos[2])
