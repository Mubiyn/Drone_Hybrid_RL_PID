...
def trajectory_tracking_reward(current_pos, target_pos, obs, action):
    ...
    speed_reward = 0.2 * np.clip(speed, 0.0, 2.0)  # Encourage movement but not too fast
    ...
    reward = tracking_reward + speed_reward + orientation_penalty + angular_penalty + control_penalty
...
def emergency_landing_reward(current_pos, target_pos, obs, action):
    ...
    reward = descent_reward + vel_penalty + lateral_penalty + orientation_penalty + gentle_landing_bonus
...
def get_reward_function(task_name):
    ...
    if task_name not in reward_functions:
        raise ValueError(f"Unknown task: {task_name}. Available: {list(reward_functions.keys())}")
