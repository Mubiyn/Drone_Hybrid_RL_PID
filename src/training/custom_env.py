...
class DroneTaskEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Denormalize action from [-1, 1] to RPM, centered at HOVER_RPM
        # action = 0 → HOVER_RPM (equilibrium)
        # action = -1 → 0 RPM (minimum)
        # action = +1 → MAX_RPM (maximum)
        action_denorm = self.hover_rpm + action * (self.env.MAX_RPM - self.hover_rpm)
        ...
        if self.wind_speed > 0:
            wind_direction = np.random.uniform(-np.pi, np.pi)
            wind_force = np.array([
                self.wind_speed * np.cos(wind_direction),
                self.wind_speed * np.sin(wind_direction),
                0.0
            ])
        ...
        done = self.trajectory_step >= len(self.trajectory) or terminated or truncated or early_termination
