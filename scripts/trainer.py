...
MODEL_PATH = os.path.join(root, "models", "waypoints_policy.zip")
...
def trainer(task, model=None, total_timesteps=500_000):
    """
    Train (or continue training) the model on a single task.
    - If `model` is None and MODEL_PATH exists → load from disk.
    - If `model` is None and no file → create a fresh PPO model.
    - Otherwise reuse given model and just swap env.
    """
    print(f"\n===== TRAINING ON TASK: {task} =====")

    # Vectorized env with 8 parallel copies of this task
    ...
    if model is None:
        ...
        #Reuse weights but change environment to this task
        model.set_env(env)
...
if __name__ == "__main__":
    task_list = ["four_points"]

    model = None
    for task in task_list:
        model = trainer(task=task, model=model, total_timesteps=2_000_000)
