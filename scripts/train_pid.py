...
def run_task(
    ...
    trajectory = get_trajectory(task_name, duration_override=duration)
    ...
    # Prepend a smooth takeoff trajectory
    if takeoff_duration > 0:
        ctrl_freq = 48

        # Define the point to ascend to: center of the trajectory's XY plane, at the target Z height
        first_target = trajectory[0]
        hover_point = np.array([initial_pos[0], initial_pos[1], first_target[2]])

        # Split takeoff into two phases: vertical ascent and transition to path
        ascent_duration = takeoff_duration * 0.6  # 60% of time for vertical
        transition_duration = takeoff_duration * 0.4  # 40% for horizontal

        ascent_steps = int(ascent_duration * ctrl_freq)
        transition_steps = int(transition_duration * ctrl_freq)

        takeoff_trajectory = np.vstack(
            [
                np.linspace(initial_pos, hover_point, ascent_steps),
                np.linspace(hover_point, first_target, transition_steps),
            ]
        )
        # Replace the first step of the original trajectory with the full takeoff
        trajectory = np.vstack([takeoff_trajectory, trajectory[1:]])

    # Append a smooth landing trajectory
    if landing_duration > 0:
        ctrl_freq = 48
        last_target = trajectory[-1]
        # Define landing spot as the initial takeoff position
        landing_spot = initial_pos
        # Define a hover point above the landing spot at the same altitude as the last target
        hover_above_landing = np.array(
            [landing_spot[0], landing_spot[1], last_target[2]]
        )

        # Split landing into two phases: transition to home, then vertical descent
        transition_duration = landing_duration * 0.4  # 40% of time for returning home
        descent_duration = landing_duration * 0.6  # 60% for vertical descent

        transition_steps = int(transition_duration * ctrl_freq)
        descent_steps = int(descent_duration * ctrl_freq)

        landing_trajectory = np.vstack(
            [
                np.linspace(last_target, hover_above_landing, transition_steps),
                np.linspace(hover_above_landing, landing_spot, descent_steps),
            ]
        )
        trajectory = np.vstack([trajectory, landing_trajectory])
    ...
    # Add a delay before starting if GUI is active
    if gui:
        print("\n[INFO] GUI is active. Simulation will start in 5 seconds.")
        time.sleep(5)
    ...
    # Add a delay before closing if GUI is active
    if gui:
        print("\n[INFO] Simulation finished. GUI will close in 5 seconds.")
        time.sleep(5)
...
def main():
    ...
    parser.add_argument(
        "--duration", type=int, default=None, help="Override task duration in seconds"
    )
    parser.add_argument(
        "--takeoff-duration",
        type=float,
        default=5.0,
        help="Duration of the initial takeoff ramp in seconds",
    )
    parser.add_argument(
        "--landing-duration",
        type=float,
        default=3.0,
        help="Duration of the final landing sequence in seconds",
    )

    # Custom parsing for key=value arguments without dashes
    argv = sys.argv[1:]
    processed_args = []
    for arg in argv:
        if "=" in arg and not arg.startswith("-"):
            ...
            if value.lower() in ["true", "1", "yes"]:
                ...
                # Handle numeric values for duration and takeoff-duration
                processed_args.extend([f"--{key}", value])
            # You could handle 'false' here if needed, but action='store_true' defaults to False
        else:
            processed_args.append(arg)
    args = parser.parse_args(processed_args)

    # Determine config file based on task if not provided
    if args.task == "circle":
        default_config = "config/pid_circle_config.yaml"
    else:
        default_config = "config/pid_hover_config.yaml"
    ...
    positions, targets, actions, results = run_task(
        controller,
        args.task,
        args.duration,
        args.takeoff_duration,
