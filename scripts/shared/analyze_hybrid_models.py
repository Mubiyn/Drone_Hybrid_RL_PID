...
def main():
    ...
    output_dir = Path('results/phase2_real_drone/model_analysis')
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"\n{'='*70}")
    print("PHASE 2: HYBRID RL MODEL ANALYSIS (RL-Only Approach)")
    print(f"{'='*70}\n")
    
    # Only analyze successful trajectories: circle, hover, spiral
    successful_trajectories = ['circle', 'hover', 'spiral']
    
    # Find rl_only models (latest for each trajectory)
    model_dirs = []
    for traj in successful_trajectories:
        traj_rl_only = sorted(logs_dir.glob(f'{traj}/rl_only_*'))
        if traj_rl_only:
            model_dirs.append(traj_rl_only[-1])  # Get latest
    
    if not model_dirs:
        print("✗ No trained models found!")
        return
    
    print(f"Found {len(model_dirs)} trained models (rl_only approach)\n")
    ...
    for model_dir in model_dirs:
        ...
        if not model_file.exists():
            print(f"⚠️  Skipping {traj_type}: model file not found")
            continue
        ...
        print(f"Evaluating {traj_type} (model: {model_dir.name})...")
        
        # Evaluate without domain randomization (baseline)
        results = evaluate_model(model_file, traj_file, n_episodes=10, domain_randomization=False)
    ...
    # Load autonomous analysis results (only for successful trajectories)
    autonomous_errors = {
        'circle': 0.3720,
        'spiral': 0.3884,
        'hover': 0.1000,
