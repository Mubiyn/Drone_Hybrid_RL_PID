...
def main():
    ...
    output_dir = Path("results/phase2_real_drone/perturbation_analysis")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Only analyze successful trajectories on Tello hardware
    # Circle, figure8, and square failed - only hover and spiral succeeded
    SUCCESSFUL_TRAJECTORIES = ['hover', 'spiral']
    print(f"Analyzing successful trajectories: {', '.join(SUCCESSFUL_TRAJECTORIES)}\n")
    ...
    for pid_file in pid_files:
        ...
        # Skip trajectories that failed on real Tello hardware
        if traj_type not in SUCCESSFUL_TRAJECTORIES:
            print(f"⏭️  Skipping {traj_type} (not successful on Tello)")
            continue
